const SkriptVersion = "0.1.11"; //vom 29.12.2020 / Link zu Git: https://github.com/Pittini/iobroker-nodemihome / Forum: https://forum.iobroker.net/topic/39388/vorlage-xiaomi-airpurifier-3h-u-a-inkl-token-auslesen
...
function RefreshDps(DeviceIndex) {
    ...
    let defidx = device[DeviceIndex].definitionindex
    //log("" + DefineDevice[defidx].common[0].read)
    //log(device[DeviceIndex].id);

    for (let x in device[DeviceIndex].data) { //Alle properties des Devices durchgehen
        for (let y in DefineDevice[defidx].common) {
            if (DefineDevice[defidx].common[y].name == CorrectChannelId(x)) {
                setState(praefix0 + "." + device[DeviceIndex].id + "." + CorrectChannelId(x), device[DeviceIndex].data[x]);
                if (logging) log("Refreshing " + praefix0 + "." + device[DeviceIndex].id + "." + CorrectChannelId(x) + " / value=" + device[DeviceIndex].data[x] + " / read=" + DefineDevice[defidx].common[y].read + " write=" + DefineDevice[defidx].common[y].write);
            };
        };
    };
    TriggerLock = false;
...
function CreateDpTrigger() {
    ...
    */
    ...
    for (let i in device) {
        ...
        let defidx = device[i].definitionindex
log("Data="+JSON.stringify(device[i].data))
        for (let x in device[i].data) { //Alle properties des Devices durchgehen
                 log(device[i].data[x]+" - "+ CorrectChannelId(x))
       
            for (let y in DefineDevice[defidx].common) {
                if (DefineDevice[defidx].common[y].name == CorrectChannelId(x)) {
                    //   setState(praefix0 + "." + device[DeviceIndex].id + "." + CorrectChannelId(x), device[DeviceIndex].data[x]);
                    if (logging) log("Setting Trigger for " + praefix0 + "." + device[i].id + "." + CorrectChannelId(x) + " / value=" + device[i].data[x] + " / read=" + DefineDevice[defidx].common[y].read + " write=" + DefineDevice[defidx].common[y].write);
                };
            };
        };
    };
