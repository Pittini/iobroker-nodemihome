const SkriptVersion = "0.1.10"; //vom 25.12.2020 / Link zu Git: https://github.com/Pittini/iobroker-nodemihome / Forum: https://forum.iobroker.net/topic/39388/vorlage-xiaomi-airpurifier-3h-u-a-inkl-token-auslesen
...
DefineDevice[0] = {
    ...
    common:
        ...
        { name: "environment.pm25-density", format: "number", read: true, write: false, min: 0, max: 600 },
...
function CorrectChannelId(cid) {
    ...
    cid = temp.replace(".", ''); //Erstmal alle potentiellen Punkte im channelnamen entfernen
...
function RefreshDps(DeviceIndex) {
    ...
    for (let x in device[DeviceIndex].data) {
        ...
        log(praefix0 + "." + device[DeviceIndex].id + "." + CorrectChannelId(x) + " - " + device[DeviceIndex].data[x]);
    };
    TriggerLock = false;
