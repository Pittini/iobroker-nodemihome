const SkriptVersion = "0.2.6"; //vom 20.01.2021 / Link zu Git: https://github.com/Pittini/iobroker-nodemihome / Forum: https://forum.iobroker.net/topic/39388/vorlage-xiaomi-airpurifier-3h-u-a-inkl-token-auslesen
...
DefineDevice[15] = { // untested
    ...
    common:
        ...
        { name: "air-purifier.fan-level", format: "number", read: true, write: true, min: 1, max: 3 },
        { name: "alarm.alarm", format: "boolean", read: true, write: true, min: false, max: true },
        { name: "indicator-light.brightness", format: "number", read: true, write: true, min: 0, max: 8 },
        { name: "indicator-light.on", format: "boolean", read: true, write: true, min: false, max: true },
        { name: "environment.pm2_5-density", format: "number", read: true, write: false, min: 0, max: 600 },
        { name: "filter.filter-life-level", format: "number", read: true, write: false, min: 0, max: 100, unit: "%" },
        { name: "filter.filter-used-time", format: "number", read: true, write: false, unit: "h" },
...
function RefreshDps(DeviceIndex, NewData) {
    ...
    //log("NewData=" + JSON.stringify(NewData))
    //log("LastData=" + JSON.stringify(device[DeviceIndex].data[0]))
    for (let x in device[DeviceIndex].data) { //Alle properties des Devices durchgehen
        for (let i in NewData) {//Alle properties des Datenblocks durchgehen
            if (NewData[i] != device[DeviceIndex].data[i]) { //Überprüfen ob Datenänderung im property
                log("New Data at " + device[DeviceIndex].model + " " + i + " oldvalue=" + device[DeviceIndex].data[i] + " newvalue=" + NewData[i])
